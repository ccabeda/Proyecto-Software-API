...
namespace Proyecto_Software_Individual.Infraestructure.Repository.Query
{
    public class RepositoryApproverRoleQuery : IRepositoryApproverRoleQuery
    {
        ...
        public async Task<List<ApproverRole>> GetAll() => await _context.ApproverRole.AsNoTracking().ToListAsync();

        public async Task<ApproverRole?> GetById(int id) => await _context.ApproverRole.FindAsync(id);
    }
}
