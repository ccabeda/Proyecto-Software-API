...
namespace Proyecto_Software_Individual.Infraestructure.Repository
{
    public class RepositoryProjectProposalQuery : IRepositoryProjectProposalQuery
    {
        ...
        public async Task Create(ProjectProposal projectProposal) => await _context.ProjectProposal.AddAsync(projectProposal);
        public async Task<List<ProjectProposal>> GetAll() => await _context.ProjectProposal.Include(p => p.AreaNavegation)
                                                                                            .Include(p => p.ProjectType)
                                                                                            .Include(p => p.ApprovalStatus)
                                                                                            .Include(p => p.User)
                                                                                               .ThenInclude(s => s.ApproverRole).ToListAsync();

        public async Task<ProjectProposal?> GetById(Guid id) => await _context.ProjectProposal.Include(p => p.AreaNavegation)
                                                                                                .Include(p => p.ProjectType)
                                                                                                .Include(p => p.ApprovalStatus)
                                                                                                .Include(p => p.User)
                                                                                                    .ThenInclude(s => s.ApproverRole).FirstOrDefaultAsync(p => p.Id == id);

        public async Task<ProjectProposal?> GetByName(string name) => await _context.ProjectProposal.FirstOrDefaultAsync(p => p.Title.ToLower() == name.ToLower());
    }
