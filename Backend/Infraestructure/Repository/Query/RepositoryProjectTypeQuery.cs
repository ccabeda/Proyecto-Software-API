...
namespace Proyecto_Software_Individual.Infraestructure.Repository.Query
{
    public class RepositoryProjectTypeQuery : IRepositoryProjectTypeQuery
    {
        ...
        public async Task<List<ProjectType>> GetAll() => await _context.ProjectType.AsNoTracking().ToListAsync();
        public async Task<ProjectType?> GetById(int id) => await _context.ProjectType.FindAsync(id);
    }
}
