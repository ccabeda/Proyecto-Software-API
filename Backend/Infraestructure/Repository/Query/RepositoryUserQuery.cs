...
namespace Proyecto_Software_Individual.Infraestructure.Repository.Query
{
    public class RepositoryUserQuery : IRepositoryUserQuery
    {
        ...
        public async Task<List<User>> GetAll() => await _context.User.Include(u => u.ApproverRole).AsNoTracking().ToListAsync(); 
        public async Task<User?> GetById(int? id) => await _context.User.FindAsync(id);
        public async Task<User?> GetUser(string email) => await _context.User.Include(u => u.ApproverRole).FirstOrDefaultAsync(u => u.Email.ToLower() == email);
    }
}
