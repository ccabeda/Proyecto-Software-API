...
namespace Proyecto_Software_Individual.Infraestructure.Repository.Query
{
    public class RepositoryAreaQuery : IRepositoryAreaQuery
    {
        ...
        public async Task<List<Area>> GetAll() => await _context.Areas.AsNoTracking().ToListAsync();
        public async Task<Area?> GetById(int id) => await _context.Areas.FindAsync(id);
    }
}
