...
namespace Proyecto_Software_Individual.Infraestructure.Repository.Query
{
    public class RepositoryApprovalStatusQuery : IRepositoryApprovalStatusQuery
    {
        ...
        public async Task<List<ApprovalStatus>> GetAll() => await _context.ApprovalStatuses.AsNoTracking().ToListAsync();

        public async Task<ApprovalStatus?> GetById(int id) => await _context.ApprovalStatuses.FindAsync(id);
    }
}
